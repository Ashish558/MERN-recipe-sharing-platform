...
router.get('/favourites', verify, async (req, res) => {
    await User.findById(req.user._id)
        .then((user) => res.status(200).json(user.favouriteRecipes))
...
router.put('/add/favourites', verify, async (req, res) => {
    const { postId } = req.body
    if (!postId) return res.status(400).json('bad request')

    await User.findById(req.user._id)
        .then((user) => {
            if (CheckItemInArray(user.favouriteRecipes, postId)) return res.status(406).json("already in favourites")

            user.favouriteRecipes.push(postId)
